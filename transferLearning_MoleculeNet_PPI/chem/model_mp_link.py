...
class GNN_mp_link(torch.nn.Module):
    def __init__(self, num_motifs, num_clusters, num_layer, emb_dim, JK = "last", 
        ...
        super(GNN_mp_link, self).__init__()
    ...
    def forward_cl(self, x, y, edge_index, edge_attr, batch, motif_idx, clique_idx, cluster_idx, device):
        x = self.gnn(x, edge_index, edge_attr)
        x = self.pool(x, batch)
        x = self.projection_head(x)
        ...
        motif_representation = self.motif_pool(x.detach().unsqueeze(1), motif_representation, None, mask)
